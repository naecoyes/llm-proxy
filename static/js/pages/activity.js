...
export function mountActivity(context) {
  ...
  const render = ({ redrawTrend = false } = {}) => {
    ...
    root.innerHTML = `<div class="page-stack">
      <div class="equal-grid"><section class="panel"><header class="panel-header"><div><h2>Active processes</h2><p>Grouped by scan_id, then legacy proxy slot and client</p></div></header><div class="panel-body">${renderProcesses(requests)}</div></section><section class="panel"><header class="panel-header"><div><h2>Stale / orphan requests</h2><p>Requests with no response after ${Math.round(STALE_PENDING_MS / 60000)} min, or missing scan context</p></div></header><div class="panel-body">${renderStalePanel(requests)}</div></section></div>
