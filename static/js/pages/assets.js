...
function renderAssetTable(data = {}) {
  ...
  return `<div class="table-wrap"><table class="data-table assets-table">
    ...
    <tbody>${rows.map((item) => `<tr>
      ...
      <td>${badge(item.last_scan_status || "unscanned")}</td>
...
export function mountAssets({ root, setFreshness, setRefreshHandler }) {
  ...
  function render() {
    ...
    root.innerHTML = `<div class="page-stack">
      ...
      ${panel(
        ...
        `${renderFilters(filters)}${loadingTable ? '<div class="muted">Loading table…</div>' : ""}<div id="assetsTableRegion">${renderAssetTable(assets)}${renderPagination(assets, filters)}</div>`
      )}
    </div>`;
    bind(assets);
  }
  ...
  async function loadInitial() {
    ...
    render();
    try {
      [summary, assets] = await Promise.all([
  ...
  setRefreshHandler(async () => {
    summary = await api.assetsSummary();
