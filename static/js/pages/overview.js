...
function renderFindingsTrendSvg(points = []) {
  ...
  const yFor = (value) => pad.top + plotH - ((Number(value || 0) - min) / (max - min || 1)) * plotH;
  const pathFor = (key) => {
    const coords = points.map((point, index) => `${index ? "L" : "M"}${xFor(index).toFixed(1)},${yFor(point[key]).toFixed(1)}`);
    if (points.length === 1) {
      const y = yFor(points[0][key]).toFixed(1);
      return `M${pad.left},${y}L${pad.left + plotW},${y}`;
    }
    return coords.join(" ");
  };
  const latest = points[points.length - 1] || {};
  const firstLabel = points[0]?.bucket ? formatDate(points[0].bucket) : "";
  const lastLabel = latest.bucket ? formatDate(latest.bucket) : "";
  const grid = [0, .25, .5, .75, 1].map((ratio) => {
    const y = pad.top + plotH * ratio;
    return `<line x1="${pad.left}" y1="${y}" x2="${pad.left + plotW}" y2="${y}" class="overview-trend-grid"/>`;
  }).join("");
  return `<div class="overview-trend-legend">${series.map(([key, label, color]) => `<span><i style="background:${color}"></i>${label}<strong>${formatNumber(latest[key] || 0, 0)}</strong></span>`).join("")}</div>
    <svg class="overview-trend-svg" viewBox="0 0 ${width} ${height}" role="img" aria-label="Findings count trend">
      ${grid}
      <text x="${pad.left}" y="${height - 8}" class="overview-trend-label">${escapeHtml(firstLabel)}</text>
      <text x="${pad.left + plotW}" y="${height - 8}" class="overview-trend-label end">${escapeHtml(lastLabel)}</text>
      <text x="${pad.left - 10}" y="${pad.top + 6}" class="overview-trend-label end">${formatNumber(max, 0)}</text>
      <text x="${pad.left - 10}" y="${pad.top + plotH}" class="overview-trend-label end">${formatNumber(min, 0)}</text>
      ${series.map(([key, , color]) => `<path d="${pathFor(key)}" fill="none" stroke="${color}" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>`).join("")}
      ${series.map(([key, , color]) => points.map((point, index) => `<circle cx="${xFor(index).toFixed(1)}" cy="${yFor(point[key]).toFixed(1)}" r="${points.length > 48 ? 0 : 3.5}" fill="${color}"><title>${escapeHtml(formatDate(point.bucket))} · ${key}: ${formatNumber(point[key] || 0, 0)}</title></circle>`).join("")).join("")}
    </svg>`;
}
...
export function mountOverview(context) {
  ...
  let lastData = null;
  let lastFindingsHistory = null;
  const render = (data, telemetryState) => {
