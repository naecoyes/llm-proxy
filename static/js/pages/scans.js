import { api } from "../api.js?v=20260623-parallel";
...
function batchRow(batch, selectedId, { compact = false } = {}) {
  ...
  const health = batchHealth(counts);
  return `<button class="batch-summary-row ${selected} ${compact ? "compact" : ""}" type="button" data-batch-id="${escapeHtml(batch.batch_id)}">
    <div class="batch-summary-main">
      ...
      <div class="batch-summary-meta">${escapeHtml(batch.scan_mode || "-")} · ${counts.total} tasks${compact ? "" : ` · ${formatDate(batch.started_at)}`}${activeTarget ? ` · active ${escapeHtml(compactTargetName(activeTarget))}` : ""}</div>
    </div>
...
function renderSubmitForm(draft) {
  return `<form id="scanSubmitForm" class="scan-submit-form">
    ...
    <div class="scan-form-grid">
      ...
      <label class="field"><span>Mode</span><select id="scanMode">${["redteam","deep","standard","quick"].map((mode) => `<option value="${mode}" ${draft.mode === mode ? "selected" : ""}>${mode}</option>`).join("")}</select></label>
      <label class="field"><span>Parallel</span><input id="scanParallel" type="number" min="1" max="4" value="${escapeHtml(draft.parallel)}"></label>
      <label class="field"><span>Timeout</span><input id="scanTimeout" type="number" min="300" max="14400" step="300" value="${escapeHtml(draft.timeout)}"></label>
    </div>
...
function formPayload(root) {
  return {
    ...
    timeout: Number(root.querySelector("#scanTimeout")?.value || 3600),
...
export function mountScans(context) {
  ...
  function render() {
    ...
    const selected = current.find((batch) => batch.batch_id === selectedId) || current[0];
    const tasks = allTasks(data);
    const currentTasks = current.flatMap((batch) => (batch.tasks || []).map((task) => ({ ...task, batch_id: batch.batch_id })));
    ...
    root.innerHTML = `<div class="page-stack scans-page">
      <div class="metrics-grid single-row scans-key-metrics">
        ...
        <div class="metric-card"><div class="metric-label">Running</div><div class="metric-value">${activeTasks.length}</div><div class="metric-detail">${current.length} current batches</div></div>
        <div class="metric-card ${dueRetryTasks.length ? "warning" : ""}"><div class="metric-label">Queued</div><div class="metric-value">${queuedTasks.length + (summary.retry_pending_tasks || 0) + autoRequeueTasks.length}</div><div class="metric-detail">${dueRetryTasks.length} ready to retry</div></div>
        ...
        <div class="metric-card ${failedTasks.length ? "warning" : ""}"><div class="metric-label">Failed</div><div class="metric-value">${failedTasks.length}</div><div class="metric-detail">${summary.timeout_tasks || 0} timed out</div></div>
      </div>
      ...
      <section class="panel"><header class="panel-header"><div><h2>Targets for selected scan</h2><p>${queueTasks.length} targets shown${selected ? ` · ${escapeHtml(selected.scan_mode || "-")}` : ""}</p></div><div class="panel-actions compact-controls"><select id="taskStatusFilter"><option value="">All</option>${[["running","Running"],["pending","Queued"],["retry","Retry"],["success","Succeeded"],["failed","Failed"]].map(([value, label]) => `<option value="${value}" ${filters.status === value ? "selected" : ""}>${label}</option>`).join("")}</select><input id="taskSearch" type="search" placeholder="Search targets" value="${escapeHtml(filters.query)}"></div></header><div class="panel-body compact-queue-body">${renderTaskList(queueTasks, { limit: 50, compact: true, emptyTitle: "No matching targets", emptyDetail: "Change the filter or select another batch." })}</div></section>
  ...
  function bind() {
    ...
    root.querySelectorAll("[data-batch-id]").forEach((button) => button.addEventListener("click", () => {
  ...
  async function previewSubmission() {
    ...
    try {
      ...
      preview.textContent = `${result.target_count} accepted targets · ${result.restricted_target_count || 0} blocked · ${result.options.mode} · parallel ${result.options.parallel} · egress ${result.options.use_socks5 ? "on" : "off"}.${blocked}`;
  ...
  async function submitScan(event) {
    ...
    const confirmed = await confirmAction({
      ...
      message: `Start this batch${dryRunText} with mode ${payload.mode}, parallel ${payload.parallel}, and egress ${payload.use_socks5 ? "enabled" : "disabled"}?${privateText}`,
  ...
  async function load(signal, { includeContainers = false } = {}) {
    ...
    const [batchData, jobData] = await Promise.all([
      api.batches(signal, 60, { includeFinished: false, includeTasks: true }),
