...
export const api = {
  ...
  findingsHistory: (signal, days = 30) => request(`/proxy/vulnerabilities/history?days=${encodeURIComponent(days)}`, { signal }),
  ...
  trend: (signal, granularity = "4h", model = "") => request(`/proxy/usage/trend?granularity=${encodeURIComponent(granularity)}${model ? `&model=${encodeURIComponent(model)}` : ""}`, { signal }),
