...
class AssetDatabase:
    ...
    def initialize(self) -> None:
        with self._migration_lock, self.transaction() as connection:
            connection.executescript(
                ...
                CREATE INDEX IF NOT EXISTS idx_asset_groups_platform ON asset_groups(platform, updated_at DESC);

                CREATE TABLE IF NOT EXISTS asset_group_members (
                    group_key TEXT NOT NULL REFERENCES asset_groups(group_key) ON DELETE CASCADE,
                    asset_id INTEGER NOT NULL REFERENCES assets(id) ON DELETE CASCADE,
                    source_ref TEXT NOT NULL DEFAULT '',
                    first_seen TEXT NOT NULL,
                    last_seen TEXT NOT NULL,
                    PRIMARY KEY(group_key, asset_id, source_ref)
                );
                CREATE INDEX IF NOT EXISTS idx_asset_group_members_asset ON asset_group_members(asset_id, last_seen DESC);

                CREATE TABLE IF NOT EXISTS target_ingest_batches (
                ...
                CREATE INDEX IF NOT EXISTS idx_target_ingest_platform ON target_ingest_batches(platform, submitted_at DESC);

                CREATE TABLE IF NOT EXISTS target_quarantine (
                    id INTEGER PRIMARY KEY,
                    ...
                    UNIQUE(platform, source_ref, raw_target, reason)
                );
    ...
    def record_target_ingest(
        ...
        *,
        ...
        group_key = f"platform:{platform_key}"
        ingest_id = f"ingest-{datetime.now(timezone.utc):%Y%m%d%H%M%S}-{uuid.uuid4().hex[:8]}"
        timestamp = now_iso()
        ...
        with self.transaction() as connection:
            connection.execute(
                ...
                ON CONFLICT(group_key) DO UPDATE SET
                  ...
                  source_ref=CASE WHEN excluded.source_ref<>'' THEN excluded.source_ref ELSE asset_groups.source_ref END,
            ...
            for target in accepted:
                ...
                connection.execute(
                    ...
                    """,
                    (group_key, asset_id, source_ref_value, timestamp, timestamp),
                )

            connection.execute(
                """
                INSERT INTO target_ingest_batches(
                  ingest_id,platform,source_ref,group_key,submitted_at,accepted_count,
                  rejected_count,job_id,dry_run,metadata_json
                )
                VALUES(?,?,?,?,?,?,?,?,?,?)
                """,
                (
            ...
            for item in rejected:
                raw_target = str(item.get("target") or item.get("raw_target") or "").strip()
                if not raw_target:
                    continue
                reason = str(item.get("reason") or item.get("category") or "rejected").strip() or "rejected"
                host = str(item.get("host") or target_host(raw_target) or "").strip()
                connection.execute(
                    """
                    INSERT INTO target_quarantine(
                      ingest_id,platform,source_ref,raw_target,host,reason,detail_json,created_at
                    )
                    VALUES(?,?,?,?,?,?,?,?)
                    ON CONFLICT(platform,source_ref,raw_target,reason) DO UPDATE SET
    ...
    def asset_groups(self) -> dict[str, Any]:
        with self.connect() as connection:
            rows = connection.execute(
                ...
                SELECT
                  g.*,
                  COUNT(DISTINCT gm.asset_id) AS asset_count,
                  MAX(gm.last_seen) AS last_seen
                FROM asset_groups g
                LEFT JOIN asset_group_members gm ON gm.group_key=g.group_key
                GROUP BY g.group_key
                ORDER BY COALESCE(MAX(gm.last_seen), g.updated_at) DESC, g.platform ASC
                """
            ).fetchall()
        return {"generated_at": now_iso(), "items": [dict(row) for row in rows]}

    def target_quarantine_page(
        self,
        *,
        platform: str = "",
        reason: str = "",
        ...
        with self.connect() as connection:
            ...
            reason_rows = connection.execute(
                "SELECT reason,COUNT(*) AS count FROM target_quarantine GROUP BY reason ORDER BY count DESC, reason ASC"
            ).fetchall()
        return {
    ...
    def sync_finding_catalog(self, records: list[dict[str, Any]]) -> None:
        """Replace finding references from the authoritative file-backed catalog."""
        target_assets: dict[str, int] = {}
        with self.connect() as connection:
            run_rows = connection.execute(
                """SELECT asset_id,path FROM artifact_refs
                WHERE artifact_type='run_dir' AND path<>''
                UNION ALL
                SELECT asset_id,source_ref FROM asset_aliases
                WHERE source_type='run' AND source_ref<>''"""
            ).fetchall()
        run_assets = {
            Path(str(row[1])).name: int(row[0])
            for row in run_rows
            if row[0] is not None and Path(str(row[1])).name
        }
        for record in records:
            target = str(record.get("target") or "").strip()
            if not target or target.lower() == "unknown":
                continue
            if target in run_assets:
                target_assets[target] = run_assets[target]
                continue
            normalized = normalize_target(target)
            if normalized["target_type"] != "domain" or "." not in normalized["target"]:
                continue
            target_assets[target] = self.upsert_asset(
                target,
                source_type="finding_catalog",
                source_ref=str(record.get("source_file") or "catalog"),
                seen_at=str(record.get("timestamp") or "") or None,
            )

        with self.transaction() as connection:
            connection.execute("DELETE FROM finding_refs")
            for record in records:
                target = str(record.get("target") or "").strip()
                file_ref = record.get("_file_ref")
                report_path = ""
                if file_ref is not None:
                    report_path = f"{getattr(file_ref, 'root_id', '')}:{getattr(file_ref, 'relative_path', '')}"
                connection.execute(
                    """INSERT INTO finding_refs(record_id,asset_id,finding_id,title,severity,cvss,source,report_path,found_at,metadata_json)
                    VALUES(?,?,?,?,?,?,?,?,?,?)""",
                    (
                        str(record.get("record_id") or ""),
                        target_assets.get(target),
                        str(record.get("id") or ""),
                        str(record.get("title") or ""),
                        str(record.get("severity") or "UNKNOWN"),
                        str(record.get("cvss") or ""),
                        str(record.get("source_file") or "catalog"),
                        report_path,
                        str(record.get("timestamp") or ""),
                        json.dumps({"target": target, "is_high_value": bool(record.get("is_high_value"))}, ensure_ascii=False),
                    ),
                )
            connection.execute(
                """UPDATE assets SET finding_count=(
                    SELECT COUNT(*) FROM finding_refs f WHERE f.asset_id=assets.id
                )"""
            )
        ...
        *,
        ...
        if platform:
            where.append(
                """
                EXISTS(
                  SELECT 1 FROM asset_group_members gm
                  JOIN asset_groups g ON g.group_key=gm.group_key
                  WHERE gm.asset_id=a.id AND g.platform=?
                )
                """
            )
            params.append(normalize_platform(platform))
        if group:
            where.append(
                "EXISTS(SELECT 1 FROM asset_group_members gm WHERE gm.asset_id=a.id AND gm.group_key=?)"
            )
    ...
    def list_assets(
        ...
        *,
        ...
        if finding_max is not None:
            where.append("(SELECT COUNT(*) FROM finding_refs f WHERE f.asset_id=a.id)<=?")
            params.append(max(0, int(finding_max)))
        clause = " WHERE " + " AND ".join(where) if where else ""
        order = {
            "target": "a.target ASC",
    ...
    def asset_detail(self, asset_id: int) -> dict[str, Any] | None:
        with self.connect() as connection:
            ...
            payload["groups"] = [dict(row) for row in connection.execute(
                """
                SELECT g.*,gm.source_ref AS member_source_ref,gm.first_seen AS member_first_seen,gm.last_seen AS member_last_seen
                FROM asset_group_members gm
                JOIN asset_groups g ON g.group_key=gm.group_key
                WHERE gm.asset_id=?
                ORDER BY gm.last_seen DESC
